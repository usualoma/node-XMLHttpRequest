...
exports.XMLHttpRequest = function() {
  ...
  this.send = function(data) {
    ...
    if (settings.method === "GET" || settings.method === "HEAD") {
      ...
      headers["Content-Length"] = typeof data === "string" ? Buffer.byteLength(data) : data.length;
    ...
    if (settings.async) {
      ...
      var execString = "var http = require('http'), https = require('https'), fs = require('fs');"
        ...
        + "fs.writeFileSync('" + syncFile + "', JSON.stringify(data), 'utf8');"
        + "}"
        + "response.on('end', function() {"
        + "write({status: response.statusCode, body: responseText});"
        + "});"
        + "response.on('error', function(error) {"
        + "write({error: error});"
        + "});"
      ...
      if (responseData.error) {
        ...
        self.responseText = responseData.body;
